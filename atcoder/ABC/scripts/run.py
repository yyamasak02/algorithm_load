...
def main():
    ...
    target_dir = f"{args.dir}/{args.target}"
    if args.pro not in ["python", "java", "rust"]:
        print(f"[ERROR] not support {args.pro}")
        return
    config = init_config(f"{args.dir}/config.yml")
    if args.target is None or not os.path.exists(target_dir):
        print(f"[INFO] running default folders: {config['default_folders']}")
        folders = [f"{args.dir}/{file}" for file in config["default_folders"]]
    else:
        folders = [target_dir]
